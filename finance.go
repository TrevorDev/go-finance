...
func GetStockInfoWithTimeOut(symbols []string, attributes []string, timeout time.Duration) (map[string]map[string]string, error) {
	...
	if len(records) < len(symbols) {
		return ret, errors.New("stock: Api request returned invalid number of symbols. " + string(len(records)) + " returned")
	}
	for i, _ := range records {
		ret[symbols[i]] = make(map[string]string)
		if len(records[i]) != len(attributes) {
			return ret, errors.New("stock: API Output attributes don't match input attributes.")
		}
		for j, _ := range records[i] {
			ret[symbols[i]][attributes[j]] = strings.TrimSpace(records[i][j])
		}
	}
